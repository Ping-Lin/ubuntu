...
"
 autocmd BufNewFile,BufRead  /synosrc/curr/ds.base/source/apparmor-2.8.x/profiles/apparmor.d-synology/* set ft=apparmor
 autocmd BufNewFile,BufRead  /synosrc/ds.base/source/apparmor-2.8.x/profiles/apparmor.d-synology/* set ft=apparmor
...
" light background
	...
	hi sdEntryIX     ctermfg=darkcyan cterm=underline
...
syn match sdHatEnd /^}\s*(#.*)?$/ contained " TODO: allow comments + [same as for syn match sdProfileEnd]
...
syn region Hat start=/\v^(\s*\^|hat\s+|profile\s+)\S+\s+((flags\s*\=\s*)?\(\s*(complain|audit|attach_disconnected|no_attach_disconnected|chroot_attach|chroot_no_attach|chroot_relative|namespace_relative|mediate_deleted|delegate_deleted)(\s*,\s*(complain|audit|attach_disconnected|no_attach_disconnected|chroot_attach|chroot_no_attach|chroot_relative|namespace_relative|mediate_deleted|delegate_deleted))*\s*\)\s+)=\{/ matchgroup=sdHatEnd end=/^\s*}\s*$/ contains=sdHatName,@sdEntry,sdComment,sdError,sdInclude
