...
function! s:GetDependProj(name)
	if b:ProjPackageMode
		let line = system(b:ProjBasePath."/pkgscripts-ng/ProjectDepends.py -x ".g:ProjDependentLevel." '".a:name."'")
		return split(line, ' ')
	endif
	...
	let line = system(b:ProjBasePath."/lnxscripts/ProjectDepends.py -x ".g:ProjDependentLevel." '".a:name."'")
	return split(line, ' ')
endfunc
...
function! ProjLoaderAuto()
	...
	if exists("b:ProjChrootPath")
		...
		call s:LoadPath(b:ProjChrootPath.'/usr/local/x86_64-pc-linux-gnu/x86_64-pc-linux-gnu/sys-root/usr/include')
	endif
